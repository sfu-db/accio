...
@Value.Enclosing
public class FedLoptOptimizeJoinRule
        extends RelRule<FedLoptOptimizeJoinRule.Config>
        implements TransformationRule {

    private static final Logger logger = LoggerFactory.getLogger(FedLoptOptimizeJoinRule.class);

    /** Creates a FederatedMultiJoinOptimizeRule. */
    protected FedLoptOptimizeJoinRule(Config config) { super(config); }

    private int[] factorSources;
    private List<Integer> addedFactors;
    private boolean ableToJoinWithPrev = false;
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        // Get source for each factor
        setFactorSources(multiJoin);
    ...
    private class TableTraitCollector extends RelVisitor {
        public RelTraitSet traitSet = RelTraitSet.createEmpty();
        ...
        @Override public void visit(RelNode node, int ordinal, @Nullable RelNode parent) {
            ...
            if (node instanceof TableScan) {
                traitSet = traitSet.merge(node.getTraitSet());
                return;
            }
            // Traverse child node
    ...
    private void setFactorSources(LoptMultiJoin multiJoin) {
        Map<RelTrait, Integer> traitHashMap = new HashMap<>();
        factorSources = new int[multiJoin.getNumJoinFactors()];
        for (int i = 0; i < multiJoin.getNumJoinFactors(); ++i) {
            RelNode factor = multiJoin.getJoinFactor(i);
            TableTraitCollector collector = new TableTraitCollector();
            collector.go(factor);
            for (RelTrait trait: collector.traitSet) {
                if (traitHashMap.containsKey(trait)) continue;
                traitHashMap.put(trait, traitHashMap.size());
            }
            if (collector.traitSet.size() > 1) {
                // If the factor contains joins of tables from different sources
                factorSources[i] = -1;
            } else {
                factorSources[i] = traitHashMap.get(collector.traitSet.getTrait(0));
            }
        }
    }

    /** The following methods are all copied from {@link LoptOptimizeJoinRule} with some modifications */
    ...
    private @Nullable LoptJoinTree createOrdering(
        ...
        while (factorsToAdd.cardinality() > 0) {
            ...
            addedFactors.add(prevFactor);
            logger.debug("Add factor: {} from source {}", prevFactor, factorSources[prevFactor]);
        }
    ...
    private int getBestNextFactor(
        ...
        boolean foundNonCrossSameSourceFactor = false;
        int lastFactor = addedFactors.get(addedFactors.size()-1);
        int [][] factorWeights = multiJoin.getFactorWeights();
        for (int factor : BitSets.toIter(factorsToAdd)) {
            boolean sameSource = factorSources[lastFactor] >= 0 && factorSources[lastFactor] == factorSources[factor];
            ...
            int dimWeight = 0;
            int i = addedFactors.size()-1;

            // If same source, first only consider set of factors joined from the same previous source
            if (sameSource) {
                for (; i >= 0; --i) {
                    int prevFactor = addedFactors.get(i);
                    if (factorSources[prevFactor] != factorSources[factor]) {
                        break;
                    }
            ...
            boolean joinableSameSource = dimWeight > 0;
            // Give higher priority for non-cross product factors from the same source with prevFactor
            // If one of such factor is found, do not consider others that are not such kind
            if (!joinableSameSource && foundNonCrossSameSourceFactor) {
                continue;
            }

            // Continue with the old logic to select a better factor
            // First compute weight (only for those that are not joinable
            if (!joinableSameSource) {
                for (; i >= 0; --i) {
                    int prevFactor = addedFactors.get(i);
                    int[] factorWeight = requireNonNull(factorWeights, "factorWeights")[prevFactor];
                    if (factorWeight[factor] > dimWeight) {
            ...
            if (joinableSameSource && !foundNonCrossSameSourceFactor) {
                nextFactor = factor;
                foundNonCrossSameSourceFactor = true;
        ...
        ableToJoinWithPrev = foundNonCrossSameSourceFactor;

        if (nextFactor < 0) {
            // factors from the same source have higher priority this case
            for (int factor : BitSets.toIter(factorsToAdd)) {
                if (factorSources[lastFactor] >= 0 && factorSources[lastFactor] == factorSources[factor]) {
                    return factor;
                }
            }
            // otherwise, simply add next factorsToAdd
            return factorsToAdd.nextSetBit(0);
        }
    ...
    private @Nullable LoptJoinTree addFactorToTree(
        ...
        if (ableToJoinWithPrev) {
            // Add to top of the right tree if factorToAdd is from the same source
            // and is able to join (not cross product) with prevFactor
            LoptJoinTree pushDownTree =
                    pushDownFactorToRight(
                            mq,
                            relBuilder,
                            multiJoin,
                            semiJoinOpt,
        ...
        // [Change]: No need to compute cost and choose the best
...
//        return bestTree;
    ...
    /**
     * Creates a join tree where the new factor is pushed down to the
     * right operand of the current join tree.
     *
     * @param multiJoin join factors being optimized
     * @param semiJoinOpt optimal semijoins for each factor
     * @param joinTree current join tree
     ...
     * returned
     */
    private @Nullable LoptJoinTree pushDownFactorToRight(
            RelMetadataQuery mq,
            RelBuilder relBuilder,
            LoptMultiJoin multiJoin,
            LoptSemiJoinOptimizer semiJoinOpt,
            ...
            List<RexNode> filtersToAdd,
            boolean selfJoin) {
        // Should not go this path if factorToAdd is a mixed source
        assert factorSources[factorToAdd] >= 0;
        ...
        // Do not further pushdown if all factors in the tree are from the same source with factorToAdd
        // XXX: Currently we do not consider any join ordering for the same source
        // may need to improve this part later
        if (!selfJoin) {
            boolean sameSourceWithTargetOnly = true;
            for (int factorInTree: joinTree.getTreeOrder()) {
                if (factorSources[factorInTree] != factorSources[factorToAdd]) {
                    sameSourceWithTargetOnly = false;
                    break;
                }
            }
            if (sameSourceWithTargetOnly) {
                return null;
            }
        }
        ...
        // recursively pushdown the factor to the right tree
        LoptJoinTree subTree = right;
        subTree =
                addFactorToTree(
                        mq,
                        relBuilder,
    ...
    /** The above methods are all copied from {@link LoptOptimizeJoinRule} with some modifications */

    /** The following methods are all copied from {@link LoptOptimizeJoinRule} */
    ...
    /**
     * NOT USED! Switch to `pushDownFactorToRight` instead!
     *
    ...
    private static LoptJoinTree createJoinSubtree(
        ...
        // [Change]: Do not need to check swap input
...
//    }
    ...
    /** The above methods are all copied from {@link LoptOptimizeJoinRule} */

    /** Rule configuration. */
    @Value.Immutable
    public interface Config extends RelRule.Config {
        Config DEFAULT = ImmutableFedLoptOptimizeJoinRule.Config.builder().build()
                .withOperandSupplier(b -> b.operand(MultiJoin.class).anyInputs());

        @Override default FedLoptOptimizeJoinRule toRule() {
            return new FedLoptOptimizeJoinRule(this);
        }
    }
}
